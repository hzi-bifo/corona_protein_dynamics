#!/usr/bin/env Rscript
buildAD_monthly <- function(infile.prefix, cutoff, thresh_color,
                    drucke, filter_sign, plotwidth, format, support_cutoff){

    #library(shape)
    mutations <- paste(infile.prefix, ".mutations.per_position.txt", sep="")
    mutmap <- paste(infile.prefix, ".subtreeMutationMap.per_position.txt", sep="")
    numiso <- paste(infile.prefix, ".numIsolates.txt", sep="")
    filename <- paste(infile.prefix, ".", format, sep="")
    sumyfile <- paste(infile.prefix, ".summary.txt", sep="") #for output
    significance <- paste(infile.prefix, ".significance.txt", sep="")    
    pvalues <- paste(infile.prefix, ".results.pvalues_fishertest.txt", sep="")
#    support <- paste(infile.prefix, ".support_values.txt", sep="")
...
#    d.support <- read.table(support, sep="\t", header=TRUE) # support values of all significant substitutions
...
#    pchVec <- rep(3, nrow(test.data))
    ...
    top <- 0.7+0.155*max_per_season  

    #define plotwidth depeding on number of months/seasons and add margin
    plotwidth <- length(d.years)*0.8 + 1.24
    ...
    axis(1,at=d.years[d.years>cutoff],labels=months,cex.axis=textSize)
	...
	for (i in index[sorting]) {
		...
		resultfile[c,1] <- months[j]		#season
    ...
    axis(3,at=d.years[d.years>cutoff],labels=months, 
...
#print("debug 11")
    ...
    names(resList) <- months[2:(length(months))]
